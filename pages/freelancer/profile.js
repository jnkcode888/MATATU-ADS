...
export default function FreelancerProfile() {
  ...
  // Update performance data to use real stats
  const [performanceData] = useState({
    ...
    audience: {
      ...
      datasets: [{
        data: [
          Math.round(stats.impressions * 0.45), // 45% Male
          Math.round(stats.impressions * 0.35), // 35% Female
          Math.round(stats.impressions * 0.20)  // 20% Mixed
        ],
        backgroundColor: [
          'rgba(54, 162, 235, 0.5)',
          'rgba(255, 99, 132, 0.5)',
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        const normalizedPreferredRoutes = profile.preferred_routes
          ? typeof profile.preferred_routes === 'string'
            ? profile.preferred_routes.split(',').map(route => route.trim()).filter(route => route)
            : Array.isArray(profile.preferred_routes)
              ? profile.preferred_routes
              : []
          : [];
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      const updates = {
        ...
        preferred_routes: formData.preferred_routes, // Supabase expects an array for text[] type
        location: formData.location || null,
        updated_at: new Date().toISOString(), // Include updated_at to satisfy database trigger
      };
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      // Step 4: Fetch gigs assigned to the freelancer
      ...
      // Step 5: Handle non-completed gigs
      for (const gig of userGigs) {
        if (gig.status !== 'completed') {
          // Fetch the campaign to restore trips_remaining
      ...
      // Step 6: Delete videos associated with non-completed gigs
      ...
      // Step 7: Delete user profile from public.users
      ...
      // Step 8: Sign out the user (optional, since auth.users is already deleted)
  ...
  const calculateProfileCompleteness = () => {
    ...
    if (userData.preferred_routes) score += 20;
        ...
        <SimpleGrid columns={[1, 2]} spacing={[4, 6]}>
          ...
          <FormControl gridColumn={["1 / -1", "1 / -1"]}>
            ...
            {editMode ? (
              ...
              <Text>{userData.preferred_routes?.join(', ') || 'Not set'}</Text>
            )}
          </FormControl>
        </SimpleGrid>
