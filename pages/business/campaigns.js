...
export default function Campaigns() {
  ...
  // Separate useEffect for fetching campaigns
  useEffect(() => {
    if (user?.id) {
      fetchCampaigns();
    }
  ...
  const handleCreateCampaign = async (campaignData) => {
    try {
      ...
      if (!userData?.phone_number) {
        // If no phone number, show phone modal
      ...
      // Create gigs using the API endpoint
      const response = await fetch('/api/create-gigs', {
        ...
        body: JSON.stringify({
          ...
          tripsNeeded: campaignData.trips_needed,
          deadline: campaignData.deadline,
          pricePerTrip: campaignData.price_per_trip,
        }),
      });

      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to create gigs');
      }
      ...
      toast({
        ...
        description: 'Your campaign has been created successfully and gigs are now available for freelancers.',
        status: 'success',
        duration: 5000,
        isClosable: true,
      });
  ...
  const handlePhoneSubmit = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Admin will contact you shortly to verify your account',
        status: 'success',
        duration: 5000,
        isClosable: true,
      });
    ...
    <Container maxW="container.xl" p={[4, 6, 8]}>
      ...
      <Modal isOpen={isPhoneModalOpen} onClose={() => setIsPhoneModalOpen(false)}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody>
            <Text mb={4}>Please add your phone number to continue. Admin will contact you to verify your account.</Text>
          ...
          <ModalBody p={8}>
            ...
            >
              ...
              <AlertDescription maxWidth="sm">
                Your campaign is now live and freelancers can start applying. You will be notified when freelancers apply for your campaign.
              </AlertDescription>
            </Alert>
          </ModalBody>
            ...
            <CardBody>
              <VStack spacing={4} align="stretch">
                ...
                <Box>
                  ...
                  <Progress value={campaign.progress || 0} colorScheme="blue" borderRadius="full" />
                </Box>

                <HStack spacing={4} justify="space-between">
                  <HStack>
                    <Icon as={FaUsers} color="gray.500" />
                    <Text fontSize="sm">{campaign.target_audience || 'All Users'}</Text>
                  </HStack>
                  <HStack>
